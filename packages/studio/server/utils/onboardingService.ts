 ...
 *
 *  1. **Service-Secret** (`NUXT_STUDIO_ONBOARDING_SECRET`) — beweist, dass der
...
function configuredSecret(event: H3Event): string {
  const config = useRuntimeConfig(event) as { studioOnboardingSecret?: string }
  return (config.studioOnboardingSecret || '').trim()
}
 ...
 * Env-Override vor Build-Default (NUXT_PUBLIC_STUDIO_POOL_PROJECT), weil das
 * Projekt pro Umgebung anders heißt.
 */
export function onboardingRuntimeProject(event?: H3Event): string {
  const config = useRuntimeConfig(event) as { public?: { studioPoolProject?: string } }
  const appConfig = useAppConfig() as { maui?: { studio?: { defaultPoolProject?: string } } }
  const projectId = (config.public?.studioPoolProject || appConfig.maui?.studio?.defaultPoolProject || '').trim()
  if (!projectId) {
