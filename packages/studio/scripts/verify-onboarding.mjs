 ...
 * Pool-User anlegen → Session → JWT → POST /api/studio/onboarding/site. Prüft
 ...
 *   node --env-file=apps/studio/.env packages/studio/scripts/verify-onboarding.mjs
 *
 * Env zusätzlich:
...
const secret = process.env.NUXT_STUDIO_ONBOARDING_SECRET
...
if (!endpoint || !controlProject || !databaseId || !controlKey || !secret) {
  console.error('✗ Env unvollständig — mit --env-file=apps/studio/.env aufrufen (und NUXT_STUDIO_ONBOARDING_SECRET setzen).')
  process.exit(1)
}
...
try {
  ...
  check('ohne Secret → 401', (await post('/api/studio/onboarding/site', payload)).status === 401)
  const wrongSecret = await post('/api/studio/onboarding/site', payload, { 'x-maui-onboarding-secret': 'falsch' })
  check('falsches Secret → 401', wrongSecret.status === 401, `war ${wrongSecret.status}`)
  const badJwt = await post('/api/studio/onboarding/site', { ...payload, jwt: 'kaputt' }, withSecret)
  check('kaputtes JWT → 401', badJwt.status === 401, `war ${badJwt.status}`)
  const badCode = await post('/api/studio/onboarding/site', { ...payload, site: { ...payload.site, inviteCode: 'MAUI-XXXX-XXXX' } }, withSecret)
  check('unbekannter Code → 403', badCode.status === 403, `war ${badCode.status}`)
  const reserved = await post('/api/studio/onboarding/site', { ...payload, site: { ...payload.site, slug: 'login' } }, withSecret)
  check('reservierter Slug (login) → 400', reserved.status === 400, `war ${reserved.status}`)
  const extra = await post('/api/studio/onboarding/site', { ...payload, site: { ...payload.site, plan: 'business' } }, withSecret)
  check('geschmuggeltes plan-Feld → 400', extra.status === 400, `war ${extra.status}`)

  console.log('\n2. Anlage')
  const created = await post('/api/studio/onboarding/site', payload, withSecret)
  ...
  const retry = await post('/api/studio/onboarding/site', payload, withSecret)
  ...
  const second = await post('/api/studio/onboarding/site', { ...payload, site: { ...payload.site, slug: `${slug}-zwei` } }, withSecret)
  ...
  const takeover = await post('/api/studio/onboarding/site', { ...payload, jwt: stranger.jwt }, withSecret)
  check('fremder Nutzer auf belegtem Host → 409 (keine Übernahme)', takeover.status === 409, `war ${takeover.status}`)

  console.log('\n4. Vorprüfung')
  const precheck = await post('/api/studio/onboarding/precheck', { code: invite.code, slug: `frei-${Date.now().toString(36)}` }, withSecret)
  ...
  const precheck2 = await post('/api/studio/onboarding/precheck', { slug }, withSecret)
  check('belegter Slug wird als belegt gemeldet', precheck2.json?.slugAvailable === false, JSON.stringify(precheck2.json))
  const precheck3 = await post('/api/studio/onboarding/precheck', { code: 'MAUI-NOPE-NOPE' }, withSecret)
