 ...
 *     packages/studio/scripts/verify-invite-stock.mjs
 */
...
try {
  ...
  const me = await callStudio('/api/studio/invites', { cookie })
  check('Betreiber sieht die Einladungs-Liste', me.status === 200, `${me.status} ${me.text.slice(0, 120)}`)
  const anon = await callStudio('/api/studio/invites')
  ...
  console.log('\n2. Vorrat auffüllen')
  const bulk = await callStudio('/api/studio/invites/bulk', {
  ...
  const afterFill = await callStudio('/api/studio/invites', { cookie })
  ...
  const assign = await callStudio(`/api/studio/invite-requests/${requestId}/assign`, { method: 'POST', cookie })
  ...
  const afterAssign = await callStudio('/api/studio/invites', { cookie })
  ...
  console.log('\n5. Erinnern')
  const remind = await callStudio(`/api/studio/invite-requests/${requestId}/assign`, { method: 'POST', cookie })
  check('erste Erinnerung geht raus', remind.status === 200, `${remind.status} ${remind.text.slice(0, 160)}`)
  const again = await callStudio(`/api/studio/invite-requests/${requestId}/assign`, { method: 'POST', cookie })
