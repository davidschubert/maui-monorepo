...
import { createCommunityMembersResolver } from '../server/utils/communityMembersResolver'
import { COMMUNITY_MEMBERS_TABLE } from '../shared/types/communityMember'
import { COMMUNITY_ROLES, communityRoleHasCapability, isCommunityRole } from '../../core/shared/communityAuthz'
...
describe.skipIf(!hasEnv)('Site-Rollen-Isolationsbeweis (echte Appwrite, community_members)', () => {
  ...
  const resolve = hasEnv
    ? createCommunityMembersResolver({ endpoint: endpoint!, projectId: projectId!, apiKey: apiKey!, databaseId: databaseId!, cacheTtlMs: 0 })
    : null!
  ...
  it('Capability-Grenzen greifen an der aufgelösten Rolle', async () => {
    ...
    expect(isCommunityRole(roleA!) && communityRoleHasCapability(roleA!, 'community.delete')).toBe(true) // owner
    expect(isCommunityRole(roleB!) && communityRoleHasCapability(roleB!, 'posts.write')).toBe(false) // viewer
  })
