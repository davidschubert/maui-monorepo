 ...
 * im Control-Dashboard (Check-in: kein Self-Service, das ist M9). Start
...
export default defineEventHandler(async (event) => {
  ...
  const row = await admin.tablesDB.createRow<WorkspaceRow>({
    ...
    rowId: ID.unique(),
    data: { name: body.name, ownerEmail: body.ownerEmail, stripeCustomerId: '', stripeSubscriptionId: '', plan: 'basic', status: 'active' },
