...
<template>
  <div class="mx-auto max-w-md py-16 text-center">
    <template v-if="state === 'working'">
      <p class="text-muted">{{ t('control.invite.working') }}</p>
    </template>

    <template v-else-if="state === 'success'">
      <UIcon name="i-ph-check-circle" class="mx-auto size-10 text-success" />
      <h1 class="mt-3 text-xl font-semibold">{{ t('control.invite.successTitle') }}</h1>
      <p class="mt-2 text-sm text-muted">{{ t('control.invite.successMessage') }}</p>
      <UButton :to="localePath('/workspace')" class="mt-6" data-invite-to-workspace>{{ t('control.invite.toWorkspace') }}</UButton>
    </template>

    <template v-else-if="state === 'needsLogin'">
      <UIcon name="i-ph-sign-in" class="mx-auto size-10 text-muted" />
      <h1 class="mt-3 text-xl font-semibold">{{ t('control.invite.needsLoginTitle') }}</h1>
      <p class="mt-2 text-sm text-muted">{{ t('control.invite.needsLoginMessage') }}</p>
    ...
    <template v-else>
      ...
      <h1 class="mt-3 text-xl font-semibold">{{ t('control.invite.invalidTitle') }}</h1>
      <p class="mt-2 text-sm text-muted">{{ t('control.invite.invalidMessage') }}</p>
    </template>
  </div>
</template>
