...
onMounted(async () => {
  ...
  try {
    ...
    toast.add({ title: t('control.invite.successTitle'), color: 'success' })
    await refresh()
  }
  catch {
...
const paidPlans = computed(() =>
  Object.entries((appConfig.pukalani as { control?: { plans?: ControlPlanCatalog } }).control?.plans ?? {})
...
async function openPortal() {
  ...
  try {
    ...
    toast.add({ title: noAccount ? t('control.customer.noBillingYet') : t('control.customer.portalFailed'), color: noAccount ? 'info' : 'error' })
...
<template>
  <UContainer class="max-w-3xl py-10">
    <h1 class="text-2xl font-bold">{{ t('control.customer.title') }}</h1>
    <p class="mt-1 text-sm text-muted">{{ t('control.customer.subtitle') }}</p>

    <p v-if="!data?.workspaces.length" class="mt-12 text-center text-sm text-muted" data-customer-empty>
      {{ t('control.customer.empty') }}
    </p>
    ...
    <div v-else class="mt-8 space-y-6" data-customer-workspaces>
      <UCard v-for="workspace in data.workspaces" :key="workspace.id" :data-customer-workspace="workspace.name">
        ...
        <div class="mt-4 space-y-2">
          <p class="text-sm font-medium">{{ t('control.customer.sites') }}</p>
          <p v-if="!workspace.sites.length" class="text-sm text-muted">{{ t('control.workspaces.noSites') }}</p>
    ...
    <UModal :open="!!planTarget" :title="t('control.workspaces.changePlanTitle', { name: planTarget?.name ?? '' })" @update:open="() => { planTarget = null }">
      <template #body>
        <div class="space-y-4">
          ...
          <UFormField :label="t('control.workspaces.interval')">
            <URadioGroup
              v-model="chosenInterval"
              orientation="horizontal"
              :items="[
                { label: t('control.workspaces.intervalMonthly'), value: 'monthly' },
                { label: t('control.workspaces.intervalYearly'), value: 'yearly' },
              ]"
              data-customer-interval-choice
            />
          </UFormField>
          <p class="text-xs text-muted">{{ t('control.customer.downgradeHint') }}</p>
        </div>
      </template>
      <template #footer>
