...
describe('Site-Zugriff: der normale Weg ist die Rolle', () => {
  it('lässt den Owner seine Seiten pflegen — ohne jedes globale Label', () => {
    const decision = decideSiteAccess({ capability: 'pages.manage', tenantScoped: true, ...OWNER })
    expect(decision).toEqual({ allowed: true, via: 'role', role: 'owner' })
  })

  it('lässt den Moderator moderieren, aber nicht schreiben', () => {
    expect(decideSiteAccess({ capability: 'comments.moderate', tenantScoped: true, role: 'moderator', labels: [] }).allowed).toBe(true)
    expect(decideSiteAccess({ capability: 'pages.manage', tenantScoped: true, role: 'moderator', labels: [] }))
      .toEqual({ allowed: false, reason: 'insufficient-role' })
  })

  it('lässt den Editor schreiben, aber nicht moderieren', () => {
    expect(decideSiteAccess({ capability: 'pages.manage', tenantScoped: true, role: 'editor', labels: [] }).allowed).toBe(true)
    expect(decideSiteAccess({ capability: 'comments.moderate', tenantScoped: true, role: 'editor', labels: [] }).allowed).toBe(false)
  })

  it('lässt den Viewer nichts verwalten', () => {
    for (const capability of ['pages.manage', 'comments.moderate', 'branding.manage'] as const) {
      expect(decideSiteAccess({ capability, tenantScoped: true, role: 'viewer', labels: [] }).allowed, capability).toBe(false)
    }
  })
})
...
describe('Fremde bleiben draußen', () => {
  it('weist ab, wer in DIESER Community keine Rolle hat', () => {
    expect(decideSiteAccess({ capability: 'pages.manage', tenantScoped: true, role: null, labels: [] }))
  ...
  it('lässt eine Owner-Rolle NICHT auf eine andere Community durchschlagen', () => {
    ...
    // woanders Owner ist.
    expect(decideSiteAccess({ capability: 'site.delete', tenantScoped: true, role: null, labels: [] }).allowed).toBe(false)
  })
})

describe('Operator-Break-Glass', () => {
  it('lässt den Betreiber durch — als operator, nicht als Rolle', () => {
    expect(decideSiteAccess({ capability: 'pages.manage', tenantScoped: true, role: null, labels: ['admin'] }))
  ...
  it('meldet Rolle statt Break-Glass, wenn die Rolle schon reicht', () => {
    ...
    expect(decideSiteAccess({ capability: 'pages.manage', tenantScoped: true, role: 'owner', labels: ['admin'] }))
  ...
  it('hilft einem Moderator-Label nicht über eine Inhalts-Capability', () => {
    ...
    expect(decideSiteAccess({ capability: 'pages.manage', tenantScoped: true, role: null, labels: ['moderator'] }).allowed).toBe(false)
    expect(decideSiteAccess({ capability: 'comments.moderate', tenantScoped: true, role: null, labels: ['moderator'] }))
      .toEqual({ allowed: true, via: 'operator' })
  })
})
...
describe('Single-Tenant-Apps bleiben unverändert', () => {
  it('entscheidet ohne Mandanten weiter über die globalen Labels', () => {
    expect(decideSiteAccess({ capability: 'pages.manage', tenantScoped: false, role: null, labels: ['admin'] }))
      .toEqual({ allowed: true, via: 'single-tenant' })
    expect(decideSiteAccess({ capability: 'pages.manage', tenantScoped: false, role: null, labels: [] }))
  ...
  it('ignoriert eine Site-Rolle ohne Mandanten-Kontext', () => {
    ...
    expect(decideSiteAccess({ capability: 'pages.manage', tenantScoped: false, role: 'owner', labels: [] }).allowed).toBe(false)
  })
})
