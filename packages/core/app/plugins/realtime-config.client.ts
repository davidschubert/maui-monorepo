...
import { parseProductsColumn, type PublicAppConfig } from '../../shared/types/config'
    ...
    useRealtimeRows<AppwriteRow & { products?: string } & Partial<Omit<PublicAppConfig, 'products'>>>(
      ...
      (event) => {
        flags.value = {
          ...
          // products-Spalte reist als JSON-String im Row-Payload mit (system-018)
          products: 'products' in event.payload ? parseProductsColumn(event.payload.products) : flags.value.products,
