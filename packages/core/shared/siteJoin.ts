 ...
 * A4 hat das Site-Label nach der Regel „wer eingeloggt einen Mandanten-Host
 * benutzt, ist Mitglied" vergeben (server/middleware/site-label.ts). Das war
 ...
 * server/middleware/site-label.ts.
 */

export type SiteJoinTrigger =
 ...
 *
 * `viewer` und keine neue Stufe: die Rollen-Matrix (shared/tenantAuthz.ts)
 ...
 */
export const SITE_JOIN_ROLE: TenantRole = 'viewer'

/** Hat dieses Ergebnis Zugang zur Community? (= Label vergeben) */
export function joinOutcomeGrantsAccess(outcome: SiteJoinOutcome): boolean {
 ...
 * nehmen).
 */
export function joinOutcomeRevokesAccess(outcome: SiteJoinOutcome): boolean {
