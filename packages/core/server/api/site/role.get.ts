/**
 * Eigene Site-Rolle des eingeloggten Users auf DIESEM Mandanten-Host (N1).
 *
 ...
 *
 * Fail-closed wie resolveTenantRole: Gast, kein Tenant, kein Resolver oder
...
export default defineEventHandler(async (event) => {
  ...
  try {
    return { role: await resolveTenantRole(event) }
  }
  catch {
    return { role: null }
  }
