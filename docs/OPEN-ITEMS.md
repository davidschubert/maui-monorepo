...
| C17 | **Der Kundenbereich hat keine Glocke** (abgespalten aus C15). Seit C15 sind kontobezogene Meldungen (`scope: 'account'`) sauber von den Communities getrennt und `/api/notifications` ist auf den Kontroll-Hosts freigegeben — **aber auf `my.pukalani.app` rendert nichts sie an**: dort gibt es nur den Wizard (`layouts/onboarding.vue`, bewusst ohne Navigation) und die Auth-Seiten; die `NotificationBell` hängt in der Chrome-Registry des **blueprint**-Layouts, also nur auf Community-Seiten. „Kommt an" ist damit halb: die API-Hälfte steht, die Anzeige fehlt. **Und es fehlt mehr als ein Layout:** die beiden `'account'`-Absender (Stripe-Webhook, Control-Invites) leben in `apps/control` — also in einem ANDEREN Appwrite-Projekt als der Pool. Ihre Zeilen landen in der `notifications`-Tabelle von `control`, die `my.pukalani.app` gar nicht liest. Der Kundenbereich zeigt kontobezogene Meldungen erst, wenn (a) es dort eine Kopfzeile mit Utilities gibt (Produkt-Entscheidung: wie sieht der Kontobereich aus — Abo, Rechnungen, Team?) UND (b) entweder `billing` im Pool verdrahtet ist (hängt an D1) oder das Control Plane seine Meldungen ins Pool-Projekt zustellt. Bis dahin sieht der Kunde Zahlungsprobleme nur per **E-Mail** (der Stripe-Zweig schickt sie) und in `control`. | C15 |
