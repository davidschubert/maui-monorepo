...
| 23 · B4 | **Ladezeit: Appwrite-Web-SDK nachladen** — David hat am 2026-08-01 Variante (a) gewählt (72 kB erst bei Bedarf, Navigations-Vorsprung bleibt). In Arbeit. | Niedrig | M | Nein — entschieden | [Notizen](#notizen) |
| 35 · F11 | **Jede Seite auth-loser Apps feuert `/api/auth/realtime-token` → 401** (B7-Fund): das Core-Realtime-Plugin läuft bedingungslos in jeder App, die core erweitert — auf der Marketing-Landing ein sinnloser Request + offener WebSocket pro Besucher. Gate oder Lazy-Start. | Niedrig | S | Nein | B7 in COMPLETE |
