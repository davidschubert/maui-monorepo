...
| 5 | **Embed-Widget E2–E4** ([Plan](plans/EMBED-WIDGET.md)) — **E2 ✅ LIVE + PROD-BEWIESEN (2026-07-23):** Schreiben im iframe über Popup-Login + verschlüsseltes Handoff-Token + CHIPS-partitioniertes Session-Cookie; CSRF-Origin-Check scharf auf comments. Beweise: Playwright `embed-write.spec.ts` (inkl. Persistenz-Assertion — Optimistic-UI kann Fehl-POSTs kaschieren) + echter Cross-Site-Lauf von **davidschubert.com/maui-embed-test.html** gegen comments.pukalani.app: Composer ohne Reload, `POST /api/comments → 201` persistiert, Cookie-Forensik zeigt Haupt-Session `Strict` unpartitioniert NEBEN Embed-Session `None; Partitioned (partitionKey=davidschubert.com)`. Task 16 (Redis-Rate-Limit) ✅. **Offen: E3-Rest** (Site-Registry `embed_sites` + Admin-UI + Registry-CSP statt `['*']`, count-API mit CORS) · E4 (Presence im Embed, Web-Component, Gast-Kommentare = Produktentscheidung) | Claude | schwer | 12 | 🔨 E2 fertig, E3 als Nächstes |
