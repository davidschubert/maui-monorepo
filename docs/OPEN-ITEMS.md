...
| C19 | **`/de` ist für englischsprachige Browser eine Endlosschleife** (gefunden 2026-07-31 beim Marketing-Aufräum-Paket, lokal reproduziert: `curl -H 'Accept-Language: en' /de` → **302 mit LEEREM Location-Header**, der Browser lädt dieselbe URL endlos neu). Betrifft NUR die Index-Route — `/de/faq → /faq` usw. leiten korrekt. Ursache liegt in der Core-i18n-Konfiguration (`detectBrowserLanguage`, `redirectOn: 'all'`), nicht in der Marketing-App; ein Fix berührt also ALLE Apps und braucht eine Prüfung gegen die dokumentierte Redirect-Strategie (CLAUDE.md i18n-Abschnitt). **Vor dem Fix auf prod reproduzieren** (`curl -H 'Accept-Language: en' https://pukalani.app/de`) — wenn es dort auftritt, läuft jeder EN-Besucher, der einen geteilten `/de`-Link öffnet, in die Schleife. | Claude |
