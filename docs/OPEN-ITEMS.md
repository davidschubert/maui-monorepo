...
| C0c | **`system-022` auf prod fahren** (`notifications.tenantId` + `idx_recipient_tenant`, aus C15). Lokal gegen die echte Instanz gefahren und per Isolationstest belegt; auf prod noch nicht. Betroffen sind **alle vier Instanzen mit Datenebene** — `control`, `platform` (Pool), `comments`, `portfolio` (system ist implizites Fundament jeder App; marketing und help haben keine Appwrite). Additiv und ruhend, Reihenfolge unkritisch: alter Code ignoriert die Spalte, neuer Code fällt ohne sie laut protokolliert auf das alte Verhalten zurück (gemischte Glocke) statt auf eine leere. **AM BESTEN VOR dem Deploy** — dann gibt es das Rückfall-Fenster gar nicht. Aufruf: `pnpm migrate --app <app> --layer system`. | C15 |
