...
| --- | --- | --- | --- | --- | --- | --- |
| F1 | **Discussions als eigenes Produkt — Schnitt-Entwurf liegt vor** (2026-08-03). Nachgemessen: gegenüber `posts` fehlen genau ZWEI Dinge — eine Kategorie-Dimension und eine danach gegliederte Ansicht; Threads, Kommentare, Stimmen und Moderation stehen dort schon. Empfehlung: Kategorie als Dimension von `posts` (Tage) statt eigenem Layer (Wochen). **Drei Entscheidungen offen: Weg A/B/C · erscheint ein kategorisierter Beitrag noch im Feed? · der Name kollidiert mit „Diskussionen" auf der Landing.** | Mittel | Tage (Weg B) bzw. XL (Weg A) | Ja: drei Entscheidungen | [DISCUSSIONS-KONZEPT.md](plans/DISCUSSIONS-KONZEPT.md) |
