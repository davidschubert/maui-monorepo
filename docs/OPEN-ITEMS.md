...
| E8 | **Umbenennung auf `community`** — Davids Entscheidung vom 2026-07-29, am 2026-07-30 im Ziel verbessert: nicht `site` gewinnt, sondern **`community`**, weil seine eigene Dashboard-Navigation die Kunden-Objekte so nennt. Plan: [UMBENENNUNG-AUF-COMMUNITY.md](plans/UMBENENNUNG-AUF-COMMUNITY.md). **Schritt 1 ist LIVE (2026-07-30): `sites` → `websites`** — Migration control-022 (Zeilen **mit ihrer Row-Id** kopiert, Gegenprobe in der Migration selbst), Tabelle/Typ/6 Routen/Seite/Antwort-Schlüssel/i18n umgestellt, prod migriert und deployt; alte Route 404, neue lebt. Die alte Tabelle `sites` steht noch; ihr Löschen braucht jetzt eine **freie Nummer ab control-024** (023 ist vergeben, siehe Schritt 2). **NICHT** mitgemacht: die Capability `sites.manage` (deckt auch Communities und Workspaces ab — wird mit E9 neu geschnitten) und `embed_sites` in comments (andere Tabelle). **Schritt 2 ist gebaut (2026-07-30, lokal grün, prod OFFEN): die Mitgliedschafts-Tabellen + der Name `siteId`** — Migration **control-023** legt `community_members`/`community_invites` an (Form 1:1, Zeilen **mit ihrer Row-Id** kopiert, Gegenprobe in der Migration; Index-Namen wandern mit: `idx_site*` → `idx_community*`) und gibt `invite_requests` additiv eine `communityId` neben der bestehenden `siteId`. Im Code heißt jedes Feld, das eine `tenants.$id` trägt, jetzt `communityId` (TenantContext, SiteMemberLookup, FormerSiteMembersLookup, SiteJoinRequest, grantSiteLabel/revokeSiteLabel/joinSite, Row-Typen `CommunityMemberRow`/`CommunityInviteRow`). **Ausrollen: Migration ZUERST, Code danach — und platform + control eng hintereinander.** Die Body-/Antwort-Felder der Service-Naht (`/api/control/site/*`, `/api/control/onboarding/site`) heißen jetzt `communityId`; solange nur EIN Deployment neu ist, antworten diese Routen 400 (Mitglieder-Seite, Branding, Registrierungs-Schalter, Wizard; `joinSite` ist fail-soft und holt es beim nächsten Auslöser nach). Eine Reihenfolge, die das vermeidet, gibt es nicht — bewusst ohne Kompatibilitäts-Brücke gebaut. Alte Tabellen + `invite_requests.siteId` bleiben stehen (Löschen ist ein eigener Schritt). **Offen: der große Teil** — `tenants` → `communities` und die 19 `tenantId`-Spalten auf bis zu vier Instanzen, danach die Zusammenführung der Autorisierungs-Zwillinge (`SiteRole`, `requireSitePermission`, `decideSiteAccess`, Site-Label, `siteTeam`-Modul — bewusst unangetastet gelassen). Läuft NICHT mehr auf A6 zu warten: A6 löscht nur workspaces\*, und keine davon wird umbenannt (Korrektur meiner früheren Behauptung). | Claude |
