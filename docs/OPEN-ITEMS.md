...
| A6 | **Die Zahlung kommt beim Kunden nicht an — `workspaces` fällt weg** (Davids Entscheidung 2026-07-29). Befund: alle vier Checkout-Routen hängen an `workspaces`, das Wort `tenant` kommt im Geldpfad NICHT vor, und `tenants.plan` (das Quota + Produkt-Sichtbarkeit steuert) wird nur vom Wizard, vom Testphasen-Sweep und vom Betreiber-Auswahlfeld geschrieben. Ein Pool-Kunde könnte also bezahlen und bliebe auf `basic` — aus den Codepfaden abgeleitet, nicht am lebenden System durchgespielt. **Entscheidung: die COMMUNITY ist das zahlende Objekt.** Damit verschwinden `workspaces`, `workspace_members` und `workspace_invites`; die Stripe-Verdrahtung wandert von control nach platform. **Korrektur meiner ersten Einschätzung:** der `entitlements`-Apparat samt Signatur-Dokumenten verschwindet NICHT mit — er ist die Lizenz-Mechanik der Studio-Seite (welche Produkte darf eine eigene Installation betreiben?), nicht Teil der Abrechnung; `workspaces` war nur sein Rechnungs-Behälter. Vorschlag: parken statt löschen. Plan mit Reihenfolge, Fallen und drei offenen Fragen: [A6-WORKSPACES-ABLOESUNG.md](plans/A6-WORKSPACES-ABLOESUNG.md). **Blockiert A2 (Stripe-Live)** — vorher Geld anzunehmen hieße, ein Versprechen zu verkaufen, das der Code nicht hält. Plan schreiben, bevor gebaut wird; berührt den Geldpfad. **Erledigt die Hälfte von E8 mit** (die doppelten Wörter verschwinden mit den Tabellen) ⇒ E8 wartet darauf. | Claude (Go: David) |
| F7 | **Bezahlte Communities** (Geldfluss 2: der Owner nimmt Geld von seinen Mitgliedern) — Davids Entscheidung 2026-07-30: **nach dem Go-Live**, in der Navigation bleibt der Platz („Payments" mit Taxes + Payment logs), das Produkt nicht. Eigene Mechanik (Stripe Connect: jeder Owner ein eigenes Stripe-Konto) und eigene rechtliche Fragen (wer ist Verkäufer, wer schuldet Umsatzsteuer, Auszahlungen, Widerruf) — das braucht Davids Zeit, nicht nur meine. Erst muss Geldfluss 1 (A6) ankommen. | geparkt |
| C18 | **Sichtbarkeit pro Community** (Davids Entscheidung 2026-07-30: wählbar, Default öffentlich). Heute sind Inhalte öffentlich — die posts-API antwortet Gästen mit 200. Der Schalter ist NICHT nur ein Feld: bei „geschlossen" müssen Row-Permissions (`read(any)` → `read(label:<communityId>)`), die SEO-Tags, `sitemap.xml`, `robots.txt` UND das og:image mitziehen — sonst steht der Inhalt in Googles Index und nicht auf der Seite. Gegenprobe muss beide Richtungen prüfen (öffentlich→geschlossen und zurück) und den Bestand mitnehmen. Nach A6/E8, weil es dieselben Tabellen anfasst. | Claude |
| ~~D7~~ | ✅ **ERLEDIGT 2026-07-30.** Neu ist `listOperatorIds()` in `packages/control/server/utils/inviteRequests.ts`: Appwrite filtert das Label jetzt SELBST (`Query.contains('labels', ['admin'])`, gegen eine laufende 1.9.6 verifiziert), dazu eine Cursor-Seitenschleife. Der `includes('admin')`-Nachcheck bleibt bewusst stehen — `contains` arbeitet auf Strings substring-artig, ein Konto mit Label `administrator` darf hier nicht mitgemeint sein. Wird der Seiten-Deckel erreicht, ist das eine LAUTE Log-Zeile statt einer stillen Kürzung. Dass die Grenze real ist, wurde beim Fix gemessen: die geprüfte Instanz hat **42 Konten**, davon 4 mit `admin` — mit `limit(25)` war das Durchrutschen keine Theorie mehr. Lint sauber, 139 Tests, Typecheck 0 Fehler. | Claude |
| ~~C19~~ | ~~**`/de` ist für englischsprachige Browser eine Endlosschleife**~~ — **Code-Fix erledigt 2026-07-31, auf prod REPRODUZIERT und lokal behoben.** Kein Konfigurationsfehler, ein Modul-Bug: `@nuxtjs/i18n` 10.6.0 baut das Redirect-Ziel per `joinURL('', '/', '/')` — ufo kollabiert lauter Schrägstriche zu `''`, genau EIN Fall betroffen (Ziel = Wurzel UND keine Query; traf auch Cookie-Kombinationen, nicht nur EN-Browser). 10.6.0 ist die einzige existierende 10.6.x-Version — kein Upstream-Patch zum Nachziehen. Fix: `packages/core/server/plugins/i18n-empty-redirect.ts` (`render:response`-Hook, normalisiert JEDEN 3xx mit leerem Location auf die App-Wurzel + repariert den meta-refresh-Body; bewusst sprachagnostisch — wird der Bug upstream behoben, wird der Handler still wirkungslos). Die dokumentierten i18n-Entscheidungen (kein fallbackLocale, redirectOn all) sind unangetastet; 10-Fälle-Matrix inkl. Crawler-Fall grün, gegen marketing UND comments verifiziert. **REST: Deploy** — der Bug sitzt über core in ALLEN Apps, jede live-Instanz braucht das nächste Release; Prod-Nachmessung danach: `curl -H 'Accept-Language: en' https://pukalani.app/de` → 302 auf `/`. | ✅ Code · Deploy offen |
...
| C2 | **UI-Plan-Gate für Kurse/Events** in der Nav (`pukalani.chrome.nav`, blueprint) — heute per Direktlink erreichbar, läuft in den API-404. | Kurse-Bericht / Audit S4 |
...
| C5 | ✅ **Der gemeldete Teil war schon erledigt** (nachgemessen 2026-07-30): `register/index.vue`, `forgot-password.vue` und `reset-password.vue` rufen alle drei `useBrandTitle(...)`. **Dafür liegt daneben eine größere, nie erfasste Lücke:** von allen Seiten in core/admin/blueprint setzen nur **9** einen Titel — **17 Dashboard-Seiten** (`dashboard/index`, `settings/*`, `users/*`, `admin/*`, `storage`, `system`) und das ÖFFENTLICHE `core/app/pages/verify.vue` setzen gar keinen, und **kein Layout springt ein**. In einer SPA heißt das nicht „kein Titel", sondern: der Titel der ZUVOR besuchten Seite bleibt im Tab stehen. Fix ist mechanisch (`useBrandTitle` je Seite, i18n-Schlüssel existieren größtenteils). | Pool-Audit B3-Rest + Nachmessung 2026-07-30 |
...
| ~~C10~~ | ✅ **ERLEDIGT — beim Nachmessen am 2026-07-30 bereits gebaut.** Der Bestätigungs-Vertrag existiert als `packages/core/app/composables/useConfirm.ts` und wird an **10** Stellen benutzt (`await confirm({...})`: Übersicht, System, Speicher, GDPR-Exporte, Changelog, Nutzer ×2, Embed, Kommentare …). Natives `window.confirm()` kommt im ganzen Repo nur noch EINMAL vor — im Docstring von `useConfirm.ts`, der beschreibt, was es ersetzt hat. Auch die Doppelklick-Sperren sind da (`saving`-Guard in `media.vue`, mit „Audit-Befund C10" annotiert). | Dashboard-Audit, UI-Hebel 2 |
| ~~C11~~ | ✅ **ERLEDIGT — beim Nachmessen am 2026-07-30 bereits gebaut.** Der Baustein heißt `packages/core/app/components/core/EmptyState.vue` (= `<CoreEmptyState>`) und steht in **22** Dateien. Die Zeile suchte nach `UEmpty` und fand deshalb nichts — die Hausform ist die eigene Komponente, nicht die Nuxt-UI-Variante; CLAUDE.md führt sie inzwischen als Standard („Leerer Zustand über CoreEmptyState"). | Dashboard-Audit, UI-Hebel 1 |
| C12b | **Neu vermessen 2026-07-30 — die Zeile war falsch beschrieben, der Rest-Fehler ist ein anderer.** „Rohes JSON" sieht nur, wer wie `curl` mit `Accept: */*` anfragt; das ist Nitros Content-Negotiation und laut Docstring in `core/server/error.ts` **bewusst so** (dort ausdrücklich als „keine Regression" festgehalten). Mit `Accept: text/html` kommt HTML. **Der echte Fehler:** dieses HTML ist Nuxts EINGEBAUTE Fehlerseite und trägt den Titel **„500 - Unknown host"**, obwohl der Status korrekt 404 ist — die gebrandete `CoreErrorPage` erscheint nicht. Gegenprobe am selben Tag: ein gewöhnlicher 404 rendert sie überall richtig (`404 · Morgenlicht`, `404 · Hawaii Studio`, `404 · Pukalani Control`, `404 · Pukalani`). Der Unterschied ist die Stelle: `core/server/middleware/00.tenant.ts:53` wirft in der SERVER-Middleware, also bevor der Renderer läuft — beim Rendern der Fehlerseite läuft dieselbe Middleware erneut und wirft wieder, weil der Host unbekannt bleibt; Nuxt fällt auf sein eingebautes Template zurück. Klein (der Host wird nirgends beworben), aber die Zahl auf der Seite ist eine Lüge. | Beobachtung 2026-07-29, korrigiert 2026-07-30 |
...
| D6 | **Das Erscheinungsbild einer Community propagiert NICHT live** (Theme, Variante und seit dem 2026-07-29 auch der Grundton). Offene Fenster übernehmen die neue Farbe erst beim nächsten Seitenaufbau — nach ≤30 s, wenn der Resolver-Cache abgelaufen ist. Das Live-Morphen, das Custom Themes und die Instanz-Einstellungen haben (`realtime-themes.client.ts`), gibt es hier nicht: `custom_themes`/`custom_fonts`/`app_config` liegen im RUNTIME-Projekt und sind `read(any)`, `tenants` liegt im **Control Plane** — ein anderes Appwrite-Projekt, in dem der Browser weder eine Session noch ein Leserecht hat. Der Text im Dashboard sagt deshalb bewusst „binnen einer halben Minute auf allen Seiten" und nicht „sofort". Möglich wäre ein Umweg über eine `read(any)`-Spiegelzeile im Runtime-Projekt (dann wieder eine zweite Wahrheit) oder ein Realtime-Kanal am Control Plane. Nicht dringend: die Farbe setzt der Owner selten, und SEIN Reload zeigt sie. | jemand die Community-Farbe oft ändert — oder ein Runtime-Spiegel ohnehin gebaut wird |
...
| E4 | **Cutover-Krümel** ([CONTROL-CUTOVER.md](runbooks/CONTROL-CUTOVER.md)): ~~ploi-Alias `studio.`~~ ✅ **entfernt 2026-07-30** (ploi → Domain aliases; `studio.pukalani.app` antwortet jetzt 404 über die Wildcard-Site `platform`, alle übrigen Hosts unverändert, verify-tls grün). Das „Doppel-Zertifikat" ist **kein Aufräum-Punkt**: control hat eine eigene Lineage, die den Alt-SAN `studio` bis zur nächsten automatischen Erneuerung mitführt — eine Anforderung nur zum Entfernen eines überzähligen Namens wäre reines Risiko ohne Nutzen. Bleibt: Read-only-Key im Projekt `control` erzeugen [David, Console]. |
| E5 | **Wellen-Migrationen**: Silo-Instanzen `photos`/`portfolio` fahren `system` mit — bei künftigen system-Migrationen mitdenken (`--wave`). |
| ~~E6~~ | ✅ **ERLEDIGT 2026-07-30** — 47 Worktrees + 47 Zweige `worktree-agent-*` entfernt (alle in `main`, ohne `--force` gelöscht, keiner hatte ungesicherte Änderungen). **Stehen geblieben und zwar bewusst:** `worktree-agent-a762b1bc42bba74d7` = der geparkte **Seiten-Editor** (1 Commit, 108 Zeilen, Feature-Stopp) und VIER Worktrees anderer Claude-Sitzungen (`claude/epic-diffie`, `claude/pukalani-landingpage-optimization`, `claude/sleepy-leavitt`, `claude/upbeat-curran`) — deren Arbeit liegt zwar in `main`, aber sie gehören mir nicht und mindestens eine ist aktiv. |
| E8 | **Umbenennung auf `community`** — Davids Entscheidung (2026-07-29/30). Plan: [UMBENENNUNG-AUF-COMMUNITY.md](plans/UMBENENNUNG-AUF-COMMUNITY.md). **Etappe 1 LIVE:** `sites` → `websites` (control-022). **Etappe 2 LIVE (2026-07-30):** `site_members`/`site_invites` → `community_members`/`community_invites` + Spalte **`siteId` → `communityId`** in beiden UND in `invite_requests` (control-023); der Name `communityId` gilt jetzt überall, wo eine `tenants.$id` transportiert wird (TenantContext, Resolver-Verträge, Service-Naht). Beweise auf meinem Baum nachgefahren: **97/97** Site-Autorisierung · **23/23** Presence-Grenze · **17/17** + **19/19** Einladungen. Beim Ausrollen bewusst KEINE Kompatibilitäts-Brücke: das Deploy-Fenster (control vor platform, dieselbe Schleife, unter einer Minute) lässt die Naht kurz mit 400 antworten — bei leeren Tabellen und ohne Kunden verhältnismäßig, eine Brücke wäre ein neuer Halbzustand gewesen. **Offen: Etappe 3** `tenants` → `communities` + die 19 `tenantId`-Spalten (echte Kundenzeilen, vier Instanzen) und **Etappe 4** die Zusammenführung von `requireTenantPermission`/`requireSitePermission` samt `SiteRole`, `decideSiteAccess`, Site-Label, `siteTeam`-Modul und den Routen-Pfaden `/api/site/**`. **Aufräumen später:** `site_members`, `site_invites`, `invite_requests.siteId` und die alte `sites` stehen noch — Löschen ab **control-024**, frühestens nach einer Nacht. | Claude |
| E11 | **Vokabular aufräumen — ein Wort je Sache** ([VOKABULAR-AUFRAEUMEN.md](plans/VOKABULAR-AUFRAEUMEN.md)) — Davids Auftrag 2026-07-30. **`feature` → `product` ist DURCH (Etappen A+B, 2026-07-30):** Migrationen control-024 + system-023 + courses-003 auf allen vier Instanzen (lokal+Prod), 964 regelbasierte Ersetzungen in 226 Dateien + 43 git mv, Typecheck 10/10 + alle Tests + Gates grün. **Offener Rest der Etappe: das ZUSAMMENZIEHEN** nach einer Nacht Beobachtung — alte Spalten/Tabellen löschen (vorher `idx_site_product` anlegen!) und die als „Übergang bis zum Zusammenziehen" markierten Dual-Writes/Aliasse entfernen; Liste im Plan. Ursprünglicher Kern war: **`feature` → `product`**, also `feature.manifest.ts` → `product.manifest.ts` (**18 Dateien**), `featureKey` (56), `featureGates` (18), `check:manifests` (13) — insgesamt **2.626 Zeilen in 413 Dateien**. **Bewusste Kehrtwende:** CLAUDE.md hält bis heute fest „im CODE bleibt das Vokabular `features`" (P4); das gilt damit nicht mehr. Zwei Stellen mit echtem Risiko: die Appwrite-Tabelle **`feature_catalog`** (kein Rename möglich ⇒ Muster control-022/023, Zeilen MIT `rowId: row.$id`, und die Row-Id ist hier der Feature-Key, der in `entitlements.featureKey` steckt) und die **öffentliche Route `/api/platform/features`**. Die Altlast **`pukalani.studio.*`** ist ebenfalls DURCH (2026-07-30: → `pukalani.control.*` samt Log-Präfixen, controlUserData, Prosa; RESERVED `studio` + Theme-Studio bleiben bewusst); übrig drei bewusste `reddit`-Kommentarreste. **Offen und Davids Entscheidung:** `maui` vs. `pukalani` — Paket-Scope `@pukalani/*`, Namespace `pukalani.*`, Repo-Name, **567 Dateien** gegen 92 mit der Marke; die teuerste Umbenennung von allen, deshalb bewusst als Frage und nicht als Aufgabe notiert. Reihenfolge zwingend seriell (alle fassen dieselben Dateien an): A6 → E8-3 → E8-4 → feature/product → pukalani.studio → E9/E10. | Claude (Go: David) |
| E10 | **Customer Feedback zentral** ([CUSTOMER-FEEDBACK.md](plans/CUSTOMER-FEEDBACK.md)) — Davids Auftrag 2026-07-30. Feedback-Knopf unten links auf JEDER Community-/Website-Seite, alles läuft zentral in den Feedback-Bereich von `control`; der Bereich hängt zugleich in ALLEN Dashboards, wo Nutzer kommentieren und hoch/runter wählen. Herkunft (anonym vs. registrierter Nutzer welcher Community) + Kategorie werden festgehalten, aus Feedback wird ein Feature Request im Board (`Under Review` → `Planned` → `In Progress` → `Complete`), sortierbar Trending/Top/New. Nav: **Management → Customer Feedback → Feedback · Roadmap (heute „Board") · Changelog**. **Befund:** `packages/feedback` und `packages/tickets` hängen heute NUR in `apps/comments` — `apps/control` extendet keinen von beiden, der Betreiber sieht die Rückmeldungen also nur, wenn er sich in einer Kunden-Silo-App anmeldet. Der LAYER-UMZUG ginge sofort; alles Weitere hängt an einer Vorab-Entscheidung: Cross-Projekt-**Lesen** aus fremden Dashboards ist dieselbe Wand wie bei D6/C17 (der Browser hat im control-Projekt weder Session noch Leserecht) — drei Wege, jeder mit Preis, im Plan aufgeschrieben. Nach E9, sonst wird das Menü zweimal gebaut. | Claude (Go + 6 Entscheidungen: David) |
| E9 | **Dashboard-Informationsarchitektur** ([DASHBOARD-IA.md](plans/DASHBOARD-IA.md)) — Davids vollständige Navigation vom 2026-07-30, geprüft gegen den Bestand: 18 Einträge stehen, 10 sind halb da, 14 existieren nirgends. Entschieden: **EINE Struktur, drei Ebenen** (Betreiber nur auf Kontroll-Hosts · Community auf ihrem Host für ihr Team · Konto überall) statt zwei getrennter Dashboards — das Fundament (`pukalani.admin.modules` mit Capability-Filter) trägt es, es kommt eine Dimension „gilt auf welcher Ebene" dazu. Zwei Einträge sind KEINE Seiten, sondern Projekte: **Custom domain** (DNS + TLS pro Kunde — die ploi-Lineage-Narbe) und **Single sign-on** (Enterprise, Studio-Seite). **Wartet auf A6 + E8**, sonst wird das Menü um ein verschwindendes und ein umbenanntes Objekt gebaut. | Claude (Go: David) |
