...
| --- | --- | --- | --- | --- | --- | --- |
| F1 | **Discussions — Stufen 1–4 live**, dazu das kleine Paket (Regeln-Vorlage für Bestandskunden · Hilfe-Umbenennung „Diskussionen"→„Kommentare" · Einstieg „Thema eröffnen" auf `/discussions`). **Als Nächstes** in Davids Reihenfolge (Konzept Teil 5): Anniversary über die Dienst-Naht · dann GEMEINSAM `posts.editedAt` + Abzeichen mehrfach + Benachrichtigung + Trust Levels · Private Nachrichten nur mit eigenem Konzept. | Mittel | je Stück Tage bis Woche | Nein (Priorisierung) | [DISCUSSIONS-KONZEPT.md](plans/DISCUSSIONS-KONZEPT.md) |
| F1 | **Discussions — Stufen 1–4 live**, dazu das kleine Paket (Regeln-Vorlage · Hilfe-Umbenennung) und der Jahrestag samt Beitritts-Zahl der About-Seite. **Als Nächstes** in Davids Reihenfolge (Konzept Teil 5): GEMEINSAM `posts.editedAt` + Abzeichen mehrfach + Benachrichtigung + Trust Levels · Private Nachrichten nur mit eigenem Konzept. | Mittel | je Stück Tage bis Woche | Nein (Priorisierung) | [DISCUSSIONS-KONZEPT.md](plans/DISCUSSIONS-KONZEPT.md) |
| F48 | 5 — Tage | **Schreibfläche für Beiträge (`UEditor`)** — der Blocker ist **weg**: der Renderer versteht seit 2026-08-04 Backslash-Escapes und HTML-Entities (CommonMark), die Bestandsdaten waren vorher gezählt (0 von 80 Texten betroffen). Offen ist nur noch die Umstellung selbst — sie ändert das Body-Format und braucht ihren eigenen Beweis (Composer + Bearbeiten teilen sich jetzt `PostBodyField`, also EINE Datei). Erwähnungen bleiben bewusst draußen. | Mittel | M | Ja: Go | [COMPOSER-UEDITOR.md](plans/COMPOSER-UEDITOR.md) |
| F1 | **Discussions — Stufen 1–4 live**, dazu das kleine Paket (Regeln-Vorlage · Hilfe-Umbenennung · Einstieg „Thema eröffnen") und der Jahrestag samt Beitritts-Zahl der About-Seite. **Jetzt dran (Davids Go 2026-08-04): das GEMEINSAME Paket** — `posts.editedAt` + Abzeichen mehrfach + Benachrichtigung + Trust Levels (TL-Architektur als strukturierte Frage). Danach: Private Nachrichten nur mit eigenem Konzept. | Mittel | Woche+ | TL-Architektur: Ja | [DISCUSSIONS-KONZEPT.md](plans/DISCUSSIONS-KONZEPT.md) |
| F1 | **Discussions — Stufen 1–4 live**, dazu kleines Paket (Regeln-Vorlage · Hilfe-Umbenennung · „Thema eröffnen"), Jahrestag + Beitritts-Zahl, und vom GEMEINSAMEN Paket (Davids Go 2026-08-04) die Teilpakete 1 (mitschreibende Zähler + `posts.editedAt` + Editor) und 2 (Mehrfach-Verleihung + Benachrichtigung). **Jetzt dran: Teilpaket 3 Trust Levels** (Architektur entschieden: TL speisen das RBAC; Schwellen nickt David ab). Danach: Private Nachrichten nur mit eigenem Konzept. | Mittel | TL: Tage | Ja: TL-Schwellen | [DISCUSSIONS-KONZEPT.md](plans/DISCUSSIONS-KONZEPT.md) |
| F48 | 5 — Tage | **Schreibfläche für Beiträge (`UEditor`)** — am 2026-08-04 gemessen und **angehalten**: der Tiptap-Serialisierer maskiert Alltagszeichen (getippt `snake_case` ⇒ gespeichert `snake\_case`), unser Renderer kennt keine Escapes und zeigt die Backslashes — 9 von 15 Alltagssätzen betroffen. Der Blocker sitzt im RENDERER, nicht im Editor. David entscheidet: „so lassen" oder „Renderer CommonMark-treu machen" (dann vorher Bestandsdaten zählen). | Mittel | M–L | Ja: Entscheidung | [COMPOSER-UEDITOR.md](plans/COMPOSER-UEDITOR.md) |
...
| F47 | 8 — Tage | **Analytics v2, Rest = nur noch Optionales** (Pakete 1–4 live seit 2026-08-04 — Schalter, Dashboard-Zahlen, Landing-Seite/Pricing, Hilfe + Datenschutz-Vorlage): Adblock-Proxy · vordefinierte Events/Goals · Plausible-Mail-Reports. Nichts davon drängt. | Niedrig | je S–M | Nein | [ANALYTICS-V2.md](plans/ANALYTICS-V2.md) |
