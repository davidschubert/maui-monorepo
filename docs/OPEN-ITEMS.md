...
| 55 · F43 | **Die Zahlungswarnung erreicht einen Community-Owner über KEINEN Kanal** (gemessen 2026-08-03). `metadata.userId` des Community-Checkouts ist eine POOL-Id, der Webhook läuft auf `control` — dort existiert sie nicht (404 nachgemessen). Die Glocken-Zeile ist unlesbar, und die E-Mail scheitert an derselben Nachschlage. Nach 14 Tagen sperrt der M13-Sweep. Der Ausfall ist seit heute LAUT (logEvent), die Zustellung braucht eine Entscheidung: Kontobereich (Konto im control anlegen?) oder Community-Glocke auf dem Mandanten-Host (berührt C15/C17)? | **Hoch** | M | Ja: wo gehört die Warnung hin? | webhook.post.ts Kopf |
