...
Stand: **2026-07-30**. Hier steht **nur, was noch offen ist** — in der
...
| 5 · E8-3 | **Umbenennung Etappe 3:** die Tabelle `tenants` heißt künftig `communities`, dazu 19 Spalten `tenantId`. Fasst echte Kundenzeilen auf vier Instanzen an. | Hoch | L — Migration auf vier Instanzen | Nein (Go steht) | [UMBENENNUNG-AUF-COMMUNITY.md](plans/UMBENENNUNG-AUF-COMMUNITY.md) |
| 6 · E8-4 | **Umbenennung Etappe 4:** die zwei Berechtigungs-Prüfungen (`requireTenantPermission` / `requireSitePermission`) werden zu einer, samt Rollen, Label und den `/api/site/**`-Pfaden. | Mittel | L — Sicherheitsnaht, viele Dateien | Nein (Go steht) | [UMBENENNUNG-AUF-COMMUNITY.md](plans/UMBENENNUNG-AUF-COMMUNITY.md) |
| 7 · E9 | **Dashboard-Navigation neu ordnen** — eine Struktur mit drei Ebenen (Betreiber · Community · Konto). 18 Einträge stehen, 10 sind halb da, 14 fehlen ganz. Erst nach 2 und 5/6. | Hoch | XL — großer Umbau, rollt auch UTable aus | Ja: Go | [DASHBOARD-IA.md](plans/DASHBOARD-IA.md) |
| 8 · E10 | **Kundenfeedback an einer Stelle sammeln** — Feedback-Knopf auf jeder Seite, zentrales Board mit Roadmap-Status. Nach 7, sonst wird das Menü zweimal gebaut. | Mittel | L — Layer-Umzug + Cross-Projekt-Frage | Ja: Go + 6 Entscheidungen | [CUSTOMER-FEEDBACK.md](plans/CUSTOMER-FEEDBACK.md) |
| 9 · E11b | **`maui` → `pukalani` im ganzen Code** (Paket-Namen, Namensraum, Repo-Name). Entschieden: Inhalt umbenennen, Ordner bleiben. 567 Dateien — die teuerste Umbenennung. | Mittel | XL — sehr breit, aber mechanisch | Nein (entschieden) | [VOKABULAR-AUFRAEUMEN.md](plans/VOKABULAR-AUFRAEUMEN.md) |
