...
| D7 | **`notifyOperators` liest nur die ersten 25 Konten** (`packages/control/server/utils/inviteRequests.ts:249`, `Query.limit(25)`) und filtert DANACH auf das Label `admin`. Ab dem 26. Konto im control-Projekt bekommen Betreiber still keine Early-Access-Anfrage mehr. Gefunden beim C17-Bau, bewusst nicht mitgefixt. Richtig ist eine gezielte Abfrage oder Pagination (Muster: `requireSiteTeamContext` nach A5). | Claude |
...
| E8 | **Umbenennung auf `community`** — Davids Entscheidung vom 2026-07-29, am 2026-07-30 im Ziel verbessert: nicht `site` gewinnt, sondern **`community`**, weil seine eigene Dashboard-Navigation die Kunden-Objekte so nennt. `tenants` → `communities`, `tenantId`/`siteId` → `communityId` (19 Tabellen), `site_members` → `community_members`, `tenant_plans` → `community_plans`, `sites` → `websites`. Damit heißt jede Sache im Code wie in der Oberfläche, die Namenskollision entfällt (`sites` wird gar nicht mehr gebraucht), und die Autorisierungs-Zwillinge `requireTenantPermission`/`requireSitePermission` werden EINE Funktion — das war der eigentliche Grund. Plan mit Bestandsaufnahme, Reihenfolge und der einen gefährlichen Stelle (Row-Id beim Kopieren): [UMBENENNUNG-AUF-COMMUNITY.md](plans/UMBENENNUNG-AUF-COMMUNITY.md). **Wartet auf A6** — sonst werden Tabellen umbenannt, die gleich gelöscht werden. | Claude (Go: David) |
