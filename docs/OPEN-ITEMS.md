...
| C6 | **Aufräum-Migration (nächste freie Nummer, system-022)**: Legacy-Spalte `app_config.entitlements` droppen — **erst wenn alle Instanzen neuen Code fahren**. (Die Nummer 021 ist seit C1b vergeben: `activities.tenantId`.) | Pool-Audit N2 |
...
| C15 | **`notifications` trägt kein tenantId** (Audit S6, bestätigt — abgespalten aus C13, weil es kein Feinschliff ist). Row-Security verhindert Fremdlesen korrekt; das Problem ist ein anderes: wer in zwei Communities Mitglied ist, sieht auf BEIDEN Hosts eine gemischte Glocke — Titel und Texte aus Community B im UI von A, und die Links zeigen auf Pfade, die es auf A nicht gibt. **Umfang:** Migration (Spalte + Index, Muster system-021 — **Nummer beim Bauen neu ziehen**, C6 und dieser Punkt zeigen beide auf 022) · Stempel in `notify()` (core, EINE Stelle — alle sechs Aufrufer gehen durch sie) · zwei Leserouten (`notifications/index.get`, `read.post`) · der `where`-Filter der Glocke (Realtime). **Drei Entscheidungen vorher [David]:** (1) Was passiert mit dem BESTAND? Im Pool haben alle vorhandenen Notifications keinen Stempel — scopte Leserouten leeren jede Glocke auf einen Schlag. Nachträglich zuordnen geht nicht ohne pro Zeile ihr Objekt nachzuschlagen. (2) Was ist mit Nachrichten OHNE Mandanten — Stripe-Webhook (Zahlung fehlgeschlagen) und Control-Plane (Early-Access-Anfrage) haben keinen Tenant-Host; brauchen sie einen expliziten `tenantId`-Parameter oder sind sie bewusst mandantenlos und überall sichtbar? (3) Die E-Mail-Zweige bauen absolute Links aus der EINEN Env-Basis (`absoluteLink`), der Digest-Sweep läuft ganz ohne Request — eine mandantenrichtige Mail braucht die Host-Auflösung des Tenants (Control-Plane, Cross-Projekt). Das ist ein eigenes Stück, nicht Teil dieser Migration. | Dashboard-Audit S6 |
| C16 | **Tote Capabilities im Dashboard** (Audit S9, ungeprüft übernommen): kein Einstieg für `branding.manage`, `posts.write`, `team.manage` (nur der Registrierungs-Schalter), `site.transfer`, `site.delete`. Ein **Editor** sieht auf platform faktisch nur Übersicht, Events und Seiten. Erst nachprüfen, dann entscheiden, ob Einstiege fehlen oder die Capabilities zu früh vergeben wurden. **Teil-Erledigung 2026-07-29:** `team.manage` und `site.transfer` haben jetzt ihren Einstieg — `/dashboard/members` (onboarding-Layer, Registry-Eintrag, Migration studio-019): einladen über EIN Feld, Rolle ändern, Zugang entziehen, Besitz übertragen. `site.delete` ist **bewusst nicht** gebaut (Davids Entscheidung 3: unumkehrbares Löschen ohne Frist erzeugt Datenverlust — auch kein gesperrter Knopf). Offen bleiben `branding.manage` (der Schalter existiert in `settings/community`, aber nur unter `team.manage` erreichbar) und `posts.write`. Beweis: `verify-site-authz.mjs` Abschnitt 8 (67/67 lokal grün). | Dashboard-Audit S9 |
