...
**A** (blockiert Umsatz) → **C0/C1b** (Sicherheit halbfertig) → **B**
...
| # | Task | Herkunft |
| --- | --- | --- |
| C0 | **Migration media-002 auf prod fahren** — sperrt unveröffentlichte Bilder zu. **Nur `comments`** (Korrektur 2026-07-28: `photos` ist NICHT in Produktion — nicht in der Deploy-Kette, `photos.pukalani.app` → 404; die Instanz lebt nur lokal. Der frühere Hinweis „photos + comments" kam aus den Manifesten, nicht aus dem Betrieb). Zugangsdaten liegen in `apps/comments/.env.production`. Live nachgemessen, die Tür ist offen: `media_items` `rowSecurity=false` + `read("any")`, Bucket `media` `fileSecurity=false` + `read("any")`. Offen ist nur, ob dieser Key Schema-SCHREIBEN darf — falls nicht, bricht die Migration beim ersten Schritt mit 401 ab und ändert nichts. ⚠️ **ERST Code deployen, DANN migrieren** (Code ist seit 2026-07-28 live). | Audit B3 |
