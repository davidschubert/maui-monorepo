...
| 4 | **Horizont 3 — Pool+Silo Multi-Tenancy** ([Blueprint](plans/HORIZONT-3-POOL-SILO-BLUEPRINT.md); Spike ✅, Schicht 1 ✅, **4.1 Pool-Datenpfad ✅**, **Naht 1/2 Auflösung ✅**, **tenants-Register + Table-Resolver + SCHARF-Beweis ✅** — Playground fährt das Gate aktiv: silo/pool/404 per Host-Header bewiesen, alle 2026-07-22). **apps/platform ✅ lokal bewiesen** (2 Pool-Kunden strikt getrennt inkl. Cache-Pfad — dabei echten Gast-Cache-Cross-Tenant-Leak gefunden + gefixt). Offen: **Platform-Prod-Rollout [David]:** Wildcard-DNS + ploi-Site + Pool-Projekt (Schritte im DECISION-LOG) · Onboarding-Flow („neue Pool-Site" = tenants-Row + Grant) · 4.2 Wellen-Migrationen (10 %) · 4.3 Quota (8 %) · Naht 4 Tenant-Label-Permissions · Silo-Admin-Key-Registry · Fläche: weitere Tabellen + tenant-aware Microcaches (changelogCache!) | Claude (Etappen-Go: David) | schwer | 40 | 🔨 in Bearbeitung |
