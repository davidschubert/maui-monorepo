...
(2026-07-31 live erwischt). ES REICHT NICHT, DEN EIGENEN SERVER AUS DEM
...
freien Port hochfahren und die Naht per Env dorthin zeigen
(`NUXT_ONBOARDING_CONTROL_URL=http://localhost:3014 pnpm --filter platform dev
--port 3016`) — danach 27/27. Ein Beweis, der über Prozessgrenzen läuft, ist
nur so ehrlich wie sein ENTFERNTESTER Dienst; ein Worktree hat außerdem weder
`node_modules` noch `.env` (installieren, `.env` aus dem Haupt-Checkout
kopieren, danach wieder löschen).
