...
if (!endpoint || !projectId || !databaseId || !apiKey) fail('apps/control/.env unvollständig (Endpoint/Projekt/DB/Key)')
...
// ── Main ────────────────────────────────────────────────────────────────────
console.log(`control-jobs · ${RUNNER_ID} · Control-Projekt ${projectId} auf ${endpoint}${watch ? ' · watch (5 s)' : ''}`)
await syncCatalog()
if (watch) {
